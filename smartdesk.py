...
def getUserHeight(faceWidth, pixelX, pixelY, nowHeight):
    ...
    calUserDistance = ((faceWidthMax - (1 - gap) * 150) - widthAverage) / faceDifference * distanceDifference + userDistanceMin
    userDistance = (calUserDistance / np.cos(userTopAngle * np.pi / 180)) / np.cos(userSideAngle * np.pi / 180)
        ...
        while True:
            ...
            if userNum == 1 and recognitionEnable == True: #인식된 얼굴 수
                ...
                if TESTMODE == False: #모니터 그래프 없이 터미널사용때
                    ...
                    print("좌측모터 PWM :" + str(int(ENA_PWM[0])))
                    print("우측모터 PWM :" + str(int(ENB_PWM[0])))
                ...
                if TESTMODE == True:
                    ...
                    val_list = [ 0 for i in range(20)] # 표준변화량을 알기 위한 어레이
            ...
            for i in range(graphRow - 1):
                ...
                recommendHeight[i] = deskMoveTall
