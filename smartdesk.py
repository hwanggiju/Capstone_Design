    ...
    try :
        ...
        while True:
            ...
            if userNum == 1 and recognitionEnable == True: #인식된 얼굴 수
                ...
                if abs(userHeightAVG - userHeight) > 10 and moveEnable == False:
                    moveEnable = True
                ...
                if moveEnable == True:
                    if waveSensorHeight + 2 < deskUserTall and stop == False: # 설정키보다 작다면
                        ...
                        fixAngleX = angleX
                        Ki_term = 0
                    elif waveSensorHeight + 2 > deskUserTall and stop == False: #설정키보다 크다면
                        stop = driverSet(100, 1, 1, 100)
                        actionPre = 0  # down
                        fixAngleY = angleY  # 현재 각도고정
                        fixAngleX = angleX
                        Ki_term = 0
                    if (waveSensorMean + 3) > deskUserTall - 5 and (waveSensorMean + 3) < deskUserTall + 5:
                        driverSet(0, 0, 0, 0)
                        stop = False
                        moveEnable == False
