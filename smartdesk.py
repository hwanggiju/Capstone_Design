...
def HorizontalHold(nowAngle, compareAngle):
    ...
    if (nowAngle-compareAngle) < 0 :
        pwmA = (20 - 5 * (1-np.cos((nowAngle-compareAngle) * np.pi/180))) * np.cos((nowAngle-compareAngle) * np.pi/180) + pwmA
        pwmB = (20 - 5 * np.cos((nowAngle-compareAngle) * np.pi/180)) * (1-np.cos((nowAngle-compareAngle) * np.pi/180)) + pwmB
    elif (nowAngle-compareAngle) > 0:
        pwmA = (20 - 5 * np.cos((nowAngle-compareAngle) * np.pi/180)) * (1-np.cos((nowAngle-compareAngle) * np.pi/180)) + pwmA
        pwmB = (20 - 5 * (1-np.cos((nowAngle-compareAngle) * np.pi/180))) * np.cos((nowAngle-compareAngle) * np.pi/180) + pwmB
    else :
        pwmA = pwmA
        pwmB = pwmB
    '''
